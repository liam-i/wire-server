...
spec :: Spec
spec = do
  describe "authIdToUserSSOId, authIdFromUserSSOId" $ do
    ...
    it "example" $ do
      ...
      authIdToUserSSOId have `shouldBe` want
      authIdFromUserSSOId want `shouldBe` Right have
    ...
    it "roundtrips" . property $
      \(x :: ValidExternalId) -> (veidFromUserSSOId @(Either String) . authIdToUserSSOId) x === Right x
